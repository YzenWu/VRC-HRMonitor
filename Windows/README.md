...
<!-- BADGES PLACEHOLDER: insert release / license / platform badges (shields.io) here -->

![Hero overview](docs/images/hero.png)
<!-- IMAGE PLACEHOLDER: main window overview — sidebar, heartbeat view, status-bar device pills -->
...
![Heartbeat curve](docs/images/heartbeat.png)
<!-- IMAGE PLACEHOLDER: heartbeat tab — large BPM readout, live curve (main / average / per-device), device list -->
...
![Pusher preview](docs/images/pusher.png)
<!-- IMAGE PLACEHOLDER: pusher tab — OSC template editor with live preview and character counter -->
...
![Floating window](docs/images/float-window.png)
<!-- IMAGE PLACEHOLDER: floating windows over a game / desktop — main window and per-device window -->
...
![Remote mobile](docs/images/remote-mobile.png)
<!-- IMAGE PLACEHOLDER: remote web frontend opened on a phone — mobile layout with heart-rate view -->
...
<!-- IMAGE PLACEHOLDER: toolkit page — dock menu expanded and the cache analyzer tool -->
...
```powershell
./build.ps1                # interactive: standalone / releases / debug
./build.ps1 --standalone   # self-contained, single-file executables
